...
public class Solution {
    public static void main(String[] args) {
        ...
        int [] interval = new int[log2(strR.length()) + 3]; 
        for (int k = 0; k < interval.length; k++) {
            interval[k] = digitsInInterval(k);
        }
        ...
        while (!lastIteration) {
            ...
            int numDigits   = interval[level + 1] - interval[level];
            ...
            if (startR == 0) {
                upper = new BigInt(strR.substring(startR, endR));
                lastIteration = true;
            } else {
                upper = BigInt.tenToPower(numDigits);
            }
            
            
            if ((!numL.equals(BigInt.ZERO) && !numL.equals(upper)) || startR == 0) {               
                BigInt count = upper.subtract(numL);
                carry = true;
                blockCount++;
